...
### 20260415（2）— 技術分析圖表 S/R 升級 & AI 報告修正
- 技術分析圖表支撐/阻力線升級為 5 條命名層級：強阻力、短線阻力、短線支撐、中線支撐、強支撐
- 1D/5D 盤中圖表改從 1M 日線計算支撐阻力線（避免盤中資料點過少導致 S/R 不準）
- 圖表下方新增 S/R 文字摘要列，解決線在可視範圍外時看不到標籤的問題
- 歐歐 AI 報告的 S/R 價位改由演算法計算後注入 prompt，不再讓 AI 自行捏造數字

### 20260415（1）— 修正新機器安裝問題
