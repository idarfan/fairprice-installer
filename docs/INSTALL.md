...
### 20260417
- 技術分析圖表支撐/阻力線升級為 5 條命名層級（S1–S2 / R1–R2 / Pivot）
- 歐歐 AI 報告的 S/R 價位改由演算法計算，不再讓 AI 自行捏造數字
- 圖表下方新增 S/R 文字摘要列，解決線在可視範圍外時看不到標籤的問題
- 1D/5D 盤中圖表改從 1M 日線計算支撐阻力線，提升準確度
- Options 頁面面板支援可調整大小（react-resizable-panels）
- Navbar 新增字體大小調整按鍵
